...
static int s_image_alpha_max(VipsInterpretation interpretation)
{
...
static VImage s_image_split(VImage in, VImage &alpha, bool &has_alpha)
{
    VImage image;
...
public:
    ...
    bool set_background_color(int r, int g, int b);
...
bool Assetry::resize(int width, int height, ResizeMode mode)
{
...
bool Assetry::crop(int width, int height, Gravity gravity)
{
    int image_width = this->image.width();
    int image_height = this->image.height();
...
bool Assetry::round(int x, int y)
{
...
bool Assetry::blur(double sigma)
{
    this->image = this->image.gaussblur(sigma, NULL);
    return true;
}

bool Assetry::set_background_color(int r, int g, int b)
{
    this->r = r;
    this->g = g;
    this->b = b;
...
void *Assetry::to_buffer(const std::string &format_in, int quality, bool strip, size_t *len)
{
    void *buf = NULL;
    std::string format;
...
extern "C"
{
    static char **s_format_list = NULL;

    static size_t s_num_formats = 0;
    ...
    bool assetry_ginit(const char *name)
    {
        ...
        if (rc)
        {
            std::list<const char *> formats = Assetry::get_formats();
    ...
    const char **assetry_get_formats(size_t *len)
    {
        *len = s_num_formats;
    ...
    Assetry *Assetry_new_from_buffer(void *buf, size_t len)
    {
        try
        {
    ...
    bool Assetry_resize(Assetry *img, int width, int height, ResizeMode mode)
    {
        try
        {
    ...
    bool Assetry_crop(Assetry *img, int width, int height, Gravity gravity)
    {
        try
        {
    ...
    {
        return img->set_background_color(r, g, b);
    }

    void *Assetry_to_buffer(Assetry *img, const char *format, int quality, bool strip, size_t *len)
    {
        try
        {
